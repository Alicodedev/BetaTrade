...
def index():
    ...
    transactions_db = db.execute("SELECT symbol, SUM(shares) AS shares_total, price FROM transactions WHERE user_id = ? GROUP BY symbol ", user_id) # querying user info based on session
    cash_db = db.execute("SELECT cash FROM users WHERE id = ? ", user_id)[0]["cash"]

    total = cash_db
    for stock in transactions_db:
        total += stock["price"] * stock["shares_total"]


    """
    if total_db:
        user_total = total_db[0]["total_balance"]
    else:
        user_total = 0  # Or any default value you want
    """

    """Show portfolio of stocks"""
    return render_template("index.html", database = transactions_db, cash = cash_db, total=total, usd_func=usd) # accessing transaction query data by using placeholders
...
def sell():
    ...
    if request.method == "POST": # checking for POST REQUEST
        ...
        user_shares = db.execute("SELECT shares FROM transactions WHERE user_id = ? AND symbol = ? GROUP BY symbol", user_id, symbol)[0]["shares"]# querying the current shares of the stock of a speific user(based on session)
        ...
        sale = shares * stock_price

        uptd_cash = (user_cash + sale)
